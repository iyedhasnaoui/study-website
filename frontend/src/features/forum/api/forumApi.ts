...
export async function getForumTopics(): Promise<ForumTopicResponseDto[]> {
    const response = await forumApiClient.get<ForumTopicResponseDto[]>('/api/forum/topics')
    return response.data
}

export async function getForumTopicById(id: number): Promise<ForumTopicResponseDto> {
    const response = await forumApiClient.get<ForumTopicResponseDto>(`/api/forum/topics/${id}`)
    return response.data
}
